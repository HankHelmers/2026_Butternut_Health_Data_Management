...
june_health_assess <- june_health_assess %>% mutate(
  ...
  plain_num = as.numeric(str_extract(height_str, "^\\d+\\.?\\d*$")),
  
  # Assume the plant_height_ft is the string as a number if the entry doesn't have text.
  plant_height_ft = if_else(!contains_text, plain_num, calculated_from_text_feet)

) %>% select(-height_str, -feet_str, -feet, -inches_str, -inches, -calculated_from_text_feet, -contains_text, -plain_num)
...
comparison <- tibble(
  ...
  original_height_ft  = original$plant_height_ft,
  clean_height_ft = june_health_assess$plant_height_ft
)
...
cleaning <- june_health_assess %>% mutate(
  # Clean up the text for consistency (e.g., remove extra spaces, make lowercase)
  dbh_str = str_to_lower(str_trim(dbh_cm)),

  # Extract the first decimal number, this will always be the cm
  # Note that broadly "\\d+\\.?\\d*" selection nomenclature simply breaks down to: get the "Digits, maybe a dot, maybe more digits"
  # Where the "\\d+" gets all the first whole digits,
  # then the "\\.?" will check whether there is a literal decimal point,
  # if there is then "\\d*" gets all remaining digits
  dbh_cm = as.numeric(str_extract(dbh_str, "\\d+\\.?\\d*")),

) %>% select(-dbh_str) # Only keep the new dbh value

cleaning <- cleaning %>% mutate(dbh_cm = as.double(dbh_cm))
...
comparison <- tibble(
  ...
  og = (june_health_assess)$dbh_cm,
  cleaned = cleaning$dbh_cm
)
...
june_health_assess <- june_health_assess %>% mutate(dbh_cm = as.double(dbh_cm))
...
row_index <- which(cleaning$plant_number == 6 & cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
...
row_index <- which(cleaning$plant_number == 17 & cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
...
row_index <- which(cleaning$plant_number == 22 & cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
...
row_index <- which(cleaning$plant_number == 38 & cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
cleaning$densio_north[row_index] <- NA
cleaning$densio_east[row_index]  <- NA
cleaning$densio_south[row_index] <- NA
cleaning$densio_west[row_index]  <- NA
...
row_index <- which(cleaning$plant_number == 43 &  cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
...
row_index <- which(cleaning$plant_number == 44 & cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
...
row_index <- which(cleaning$plant_number == 47 & cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
...
row_index <- which(cleaning$plant_number == 71 & cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
...
row_index <- which(cleaning$plant_number == 72 & cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
...
# Plant WCP 30 
  # Read: "Densiometer measures not relevant because seedling was overtaken by honeysuckle. Honeysuckle was cut away before assessing."
row_index <- which(cleaning$plant_number == 30 & cleaning$site_name == "WCP" & year(cleaning$timestamp) == 2025)
cleaning$densio_north[row_index] <- NA
cleaning$densio_east[row_index]  <- NA
cleaning$densio_south[row_index] <- NA
cleaning$densio_west[row_index]  <- NA
...
# # Align cleaning with june_health_assess (character)
# cleaning <- cleaning %>%
#   mutate(
#     densio_north = as.character(densio_north),
#     densio_south = as.character(densio_south),
#     densio_east  = as.character(densio_east),
#     densio_west  = as.character(densio_west)
#   )

# Update all values 
june_health_assess <- june_health_assess %>% rows_update(cleaning, by = c("timestamp", "plant_number", "site_name"))

# Turn back to numbers
