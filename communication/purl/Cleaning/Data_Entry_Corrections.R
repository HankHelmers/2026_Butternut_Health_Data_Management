...
individuals_timestamp = ymd_hms("2025-07-11T11:03:41Z")

# Implement changes
row_index <- which(cleaning$timestamp == individuals_timestamp)
cleaning$gps_north[row_index] <- 44.332612
...
comparison <- tibble(
  ...
  og_gps_west = (after_health_assess %>% filter(timestamp == individuals_timestamp))$gps_north,
  cleaned = (cleaning %>% filter(timestamp == individuals_timestamp))$gps_north
)
